#### 1.  **请问提交PR后出现boostkit-cla/no红色标签，如何处理？**


出现该标签表示该PR中所包含的commit中，有部分贡献者没有签署BoostKIt社区的贡献者协议CLA。签署地址可以在PR评论区找到。如果是个人贡献者请选择“签署个人CLA”，如果是作为企业管理员参与贡献请选择“企业管理员”签署CLA。CLA检查是使用commit信息中的commit邮箱作为检查凭证的。该邮箱可以通过git log --pretty=fuller查询到。
...
<td>调整Gitee提交邮箱为commit邮箱，在GitCode个人设置页面添加commit邮箱并设置为提交邮箱，然后在上述签署地址完成CLA签署即可</td>
</tr>
<tr>
<td>希望使用CitCode提交邮箱签署</td>
...
#### 2.  **请问为什么不能fork一个BoostKit/abc仓库到个人账号下？**

这个问题通常是因为在您的个人账号下，已经有abc的同名仓库，比如你之前已经从BoostKit组织下fork了名称为abc的仓库；因为码云是通过你的个人账号加仓库名寻址的，所有不允许在你的个人账号下有同名仓库。

解决方法：

1、修改个人账户下已有仓库的名称和路径，然后再从“BoostKit/abc”仓库fork即可
...
#### 6.  **请问BoostKit社区仓库评论区都支持哪些命令，分别都是什么含义？**


目前社区仓库评论区主要支持的命令请参见BoostKit社区评论命令一览。
